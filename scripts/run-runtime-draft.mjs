...
const SUPPORTED_MODES = new Set(["dry-run", "null-runner", "synthetic", "openai"]);

function printHelp() {
  console.log(`Usage: pnpm validate:runtime:draft <fixture-path> [--case-id <id> | --case-index <n>] [--mode dry-run|null-runner|synthetic|openai] [--format json]\n\nRun the standalone runtime draft CLI for a single fixture and single replay case. This entry only orchestrates the current static -> preflight -> single-case runtime skeleton chain and prints one runtime draft JSON artifact to stdout.\n\nOptions:\n  --case-id <id>        Select a specific replay case by id\n  --case-index <n>      Select a specific replay case by zero-based index\n  --mode <mode>         Runtime draft mode: dry-run (default), null-runner, synthetic, or openai\n  --format json         Output JSON (default; currently the only supported format)\n  --help, -h            Show this help message\n\nExit codes:\n  0  Successfully produced a runtime draft artifact\n  1  Runtime draft orchestration failed and emitted an error artifact\n  2  CLI usage error\n\nImportant:\n  CLI success does not mean runtime passed. In this phase the runtime draft artifact is still skeleton-only and must remain honest about blocked/dry-run/not-executed states. The openai mode makes real HTTP calls to the OpenAI API when OPENAI_API_KEY is set, or returns a clear missing-key error when it is not.\n`);
}
...
async function buildRuntimeDraftArtifact({ fixturePath, caseId, caseIndex, mode, format }) {
  ...
  if (mode === "openai") {
    // OpenAI mode uses the real execution path; we need to load the fixture
    // and pass through preflight but bypass the skeleton for the real API call.
    // For now, delegate to the async real execution variant.
    const { loadFixture } = await import("../src/skillforge/loader.mjs");
    const { normalizeFixture } = await import("../src/skillforge/normalize.mjs");
    const { validateFixture } = await import("../src/skillforge/validator.mjs");
    const { validatePreflight } = await import("../src/skillforge/preflight-validator.mjs");

    const loadedFixture = await loadFixture(fixtureDir);
    const normalizedFixture = normalizeFixture(loadedFixture);
    const staticReport = await validateFixture(fixtureDir, { format });
    const preflightReport = validatePreflight({
      fixtureDir, loadedFixture, normalizedFixture, staticReport, options: { format },
    });

    const runtimeDraft = await runRuntimeCaseSkeletonRealExecution({
...
async function main() {
  ...
  if (!SUPPORTED_MODES.has(parsed.mode)) {
    console.error(`Unsupported mode: ${parsed.mode}. Supported modes: dry-run, null-runner, synthetic, openai.`);
    process.exit(EXIT_USAGE_ERROR);
  }
