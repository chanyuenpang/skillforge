...
- provider-backed reserved-slot fields remain contract-tightened placeholders only: fields such as provider execution metadata, provider transcript bits, persistence handles, raw-response handles/evidence semantics, and provider evidence availability must currently remain `false`, `null`, `"none"`, or other explicit reserved values rather than implying provider execution; `transcriptAvailability` plus `transcriptRef.handle / providerManaged / providerTranscript / persistence` and `providerMetadata.transcriptPersistence` are also reserved-only transcript seam fields and must not be read as real provider transcript capture/retrieval/persistence; `availability` is not `persistence`, and `persistence` is not evidence availability; `rawResponse` remains a summary-only / reserved-only minimal truth slot rather than a captured provider payload or payload retrieval handle; the one narrow exception is that provider-backed reserved seam artifacts may now carry a same-source stub `executionId / providerRunId / providerStatus` tuple as conservative metadata, but that tuple still must not imply real provider execution, a real provider request/run, or any public status expansion;
...
- Within the provider-backed reserved seam, that same-source truth payload may now also surface a stub `executionId / providerRunId / providerStatus` tuple. This tuple is conservative execution metadata only: it is reserved for future real provider execution identity/status plumbing and must not be interpreted as `executed=true`, `providerCall=true`, provider evidence, transcript/persistence availability, or proof that a real provider request/run already happened.
...
- Within that contract, `rawResponse` is only the minimal skeleton/truth-payload slot. It is not a full provider payload capture, not a transcript, not a persistence/retrieval handle, and not a provider payload retrieval handle. If `rawResponse.summary` is present, it is only a summary slot.
...
- Any emitted transcript evidence must continue to declare `providerTranscript=false`, `providerManaged=false`, and a reserved/non-persistent handle/persistence state, and must not claim provider execution.
- `transcriptAvailability`, `transcriptRef.handle / providerManaged / providerTranscript / persistence`, and `providerMetadata.transcriptPersistence` therefore remain reserved-only contract fields: they describe where a future real provider transcript contract would attach, not that one already exists.
- In this reserved seam, `transcriptAvailability` does not prove transcript persistence, `providerManaged` does not prove retrieval support, and `persistence` does not prove evidence availability.
...
- Provider-backed reserved-slot fields must remain honest in the emitted artifact: `summary.passed` stays `false`; `rawResponse` remains same-source minimal skeleton passthrough rather than proof of a real provider call, full provider payload capture, or payload retrieval; `providerExecution` stays reserved/unimplemented for true provider-backed execution except that it may now expose a same-source stub `executionId / providerRunId / providerStatus` tuple as conservative metadata only, and that tuple remains confined to metadata/execution seam rather than becoming a new public case status token; `transcriptAvailability` stays reserved/non-provider and does not itself prove persistence; provider transcript handles stay unavailable; `providerMetadata.transcriptPersistence` and transcript persistence-related fields stay reserved/unimplemented; persistence stays reserved/unimplemented; and provider evidence availability must not be set true.
- Non-provider-backed paths must still force `executionId`, `providerRunId`, and `providerStatus` back to `null`; downstream layers must not synthesize or partially reconstruct the tuple from mixed sources.
- The same fallback rule applies to raw-response-related reserved seam fields on non-provider-backed paths: they must remain unavailable/`null`/summary-less as applicable rather than being inferred from transcript stubs, execution metadata stubs, or any other mixed source.
- The same fallback rule applies to transcript-related reserved seam fields on non-provider-backed paths: they must remain `available=false`, `providerManaged=false`, `providerTranscript=false`, `handle=null`, `transcriptPersistence=false`, and persistence `"none"`/reserved as applicable, rather than being inferred from provider-less stubs or any other mixed source.
...
- Scoring reserved stub 已冻结（`buildScoringStub`、`buildScoringStubProviderReserved`、`assertScoringStubContract`、`SCORING_MODES`），但这只表示 reserved seam 的代码骨架存在，不表示真实 scoring / rubric engine 已实现。`scored` 在当前阶段永远为 `false`；`scoringMode` 只允许 `"none"` 和 `"reserved-pending"` 两个值。在当前阶段，任何试图将 `scored` 设为 `true` 的行为都会被 `assertScoringStubContract` guard 拒绝。当前不开放 `passed` 状态。
- Transcript capture reserved skeleton 已冻结（`captureProviderTranscriptStub`、`assertTranscriptCaptureStubContract`、`TRANSCRIPT_CAPTURE_MODES`），但这只表示 reserved seam 的代码骨架存在，不表示真实 transcript capture/retrieval/persistence 已实现。`captured` 在当前阶段永远为 `false`；`captureMode` 只允许 `"none"` 和 `"reserved-provider-captured"` 两个值。在当前阶段，任何试图将 `captured` 设为 `true` 的行为都会被 `assertTranscriptCaptureStubContract` guard 拒绝。
- Sandbox enforcement reserved stub 已冻结（`buildSandboxStub`、`buildSandboxStubProviderReserved`、`assertSandboxStubContract`、`SANDBOX_MODES`），但这只表示 reserved seam 的代码骨架存在，不表示真实 sandbox isolation / timeout / resource limits 已实现。`enforced` 在当前阶段永远为 `false`；`sandboxMode` 只允许 `"none"` 和 `"reserved-pending"` 两个值。在当前阶段，任何试图将 `enforced` 设为 `true` 的行为都会被 `assertSandboxStubContract` guard 拒绝。
- Multi-case orchestration reserved stub 已冻结（`buildMultiCaseStub`、`buildMultiCaseStubProviderReserved`、`assertMultiCaseStubContract`、`CASE_MODES`），但这只表示 reserved seam 的代码骨架存在，不表示真实 multi-case parallel/batch execution 或 case dependency orchestration 已实现。`caseMode` 在当前阶段永远为 `single`；`CASE_MODES` 只允许 `"single"` 和 `"reserved-multi"` 两个值。在当前阶段，任何试图将 `caseMode` 设为 `"reserved-multi"` 以外的无效值的行为都会被 `assertMultiCaseStubContract` guard 拒绝。
### AcceptanceRecord contract（runtime acceptance wiring）

Phase 3 runtime acceptance wiring 已完成最小 AcceptanceRecord 合同骨架。它**不是**真实 acceptance/grading，也**未接入** `validate` 主链路。

Acceptance 相关的 provenance 语义也要明确：`acceptance.provenanceSummary` 只负责透传 runtime/source 的 human-readable 来源说明，不能被当作 pass/fail 证据，不能反推真实执行，不能参与 acceptance 决策。

对齐到 runtime report 的来源枚举时，`metadata.executionSource` 仅允许表达 `provider-synthetic` / `provider-reserved` / `provider-less` 三类诚实来源：`provider-synthetic` 是 synthetic-mock，`provider-reserved` 是 reserved-unimplemented，`provider-less` 是 provider-less-draft。它们都不是通过验证的真实执行证明。

| 组件 | 状态 | 说明 |
| --- | --- | --- |
| `ACCEPTANCE_STATUSES` | ✅ frozen | 允许值：`pending`、`blocked`、`accepted-reserved`。不包含 `passed`/`failed` 等真实验收结论。 |
| `buildAcceptanceFromRuntimeReport` | ✅ frozen | 从 runtime replay report 构建 AcceptanceRecord 的 builder。仅映射草案级字段，不引入真实评分。 |
| `assertAcceptanceRecordContract` | ✅ frozen | 合同 guard：`accepted-reserved` 在当前阶段会被拒绝（guard throws），确保无人误用 `accepted-reserved` 宣称验收通过。 |

边界声明：

- ⚠️ **NOT real acceptance / grading**：AcceptanceRecord 当前只是合同骨架，`accepted-reserved` 被 guard 拒绝，不代表真实验收评分已完成。
- ⚠️ **NOT 接入 validate 主链路**：AcceptanceRecord 不接入 `pnpm validate`、`validate:all`、`validate:contracts`、`validate:preflight` 或 `validate:runtime:draft` 的默认输出。它是独立合同单元，仅供后续真实 provider-backed 验收流程消费。
- AcceptanceRecord 的 `status` 在当前阶段实际上只允许落在 `pending | blocked`；`accepted-reserved` 虽然在枚举中冻结，但 `assertAcceptanceRecordContract` 会在运行时阻止其使用。
