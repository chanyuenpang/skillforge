...
const RUNNER_FAILURE_STATUS_SEMANTICS = Object.freeze({
  ...
  "synthetic-passed": Object.freeze({
    precedenceRank: 4,
    requiresFailureReasonCodes: [],
    meaning: "runner must use synthetic-passed only for the explicit synthetic provider mock path after preflight passed; produces observable payload without real provider execution",
  }),
});
...
const DEFAULT_PROVIDER_ADAPTER_SEAM = Object.freeze({
  ...
  futureRequiredFields: [
    ...
    "providerMetadata.executionIdentityKind",
    "providerMetadata.executionIdentityVersion",
...
const RUNNER_TRANSCRIPT_REF_PERSISTENCE_MODES = Object.freeze(["none", "in-report-only", "reserved-provider-managed"]);
...
export function buildRuntimeRunnerResult({
  ...
  const normalizedRunnerMetadata = {
    ...
    providerExecutionIdentityKind: runnerMetadata?.providerBacked === true ? PROVIDER_EXECUTION_IDENTITY_KIND : null,
    providerExecutionIdentityVersion: runnerMetadata?.providerBacked === true ? PROVIDER_EXECUTION_IDENTITY_VERSION : null,
    futureProviderRequiredFields: [
      "runnerMetadata.providerExecutionIdentityKind",
      "runnerMetadata.providerExecutionIdentityVersion",
  ...
  const normalizedTranscriptRef =
    transcriptRef && typeof transcriptRef === "object" && !Array.isArray(transcriptRef)
      ? {
          ...
          persistence: transcriptRef?.persistence ?? "none",
        }
      : null;

  if (normalizedTranscriptRef && !RUNNER_TRANSCRIPT_REF_PERSISTENCE_MODES.includes(normalizedTranscriptRef.persistence)) {
    throw new RangeError(`Unsupported runtime runner transcript ref persistence mode: ${normalizedTranscriptRef.persistence}`);
  }
  ...
  if (normalizedRunnerMetadata.providerBacked !== true && normalizedTranscriptRef?.providerManaged === true) {
    throw new RangeError("Provider-managed transcript refs are reserved for provider-backed runtime execution only");
  }
